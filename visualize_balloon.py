...
from detectron2.utils.visualizer import ColorMode
#from build_model.py import cfg
...
#load model
cfg = get_cfg()	#gets a copy of the default config
...
#dataloader = build_detection_train_loader(model, mapper=DatasetMapper, total_batch_size=76)
...
def custom_mapper(dataset_dict2, filename):
    ...
    print("dataset_dict3", dataset_dict3)
    # can use other ways to read image
    #image = utils.read_image(dataset_dict3["file_name"], format="BGR")
    ...
    annos = [
        ...
        for annotation in dataset_dict3.pop("annotations")
    ]
    print('')
    print('Custom Mapper Output')
    print("Image: ", image)
    print("annos: ", annos)
    print('')
    return {
...
for d in ["train", "val"]:
	...
	#print(predictor(d))
	#print(predictor["instances"].pred_classes)
	DatasetCatalog.register("balloon_" + d, lambda d=d: get_balloon_dicts( d))
...
ast_metadata = MetadataCatalog.get("balloon_train")
...
#dataset_dicts = get_balloon_dicts(curdir + "/balloon/train")
...
#testing to see if I built the model correctly
#out = model(dataset_dicts)
#print(out)

#dataloader_train = build_detection_train_loader("balloon_train", mapper=custom_mapper)
#dataloader = build_detection_train_loader(dataset = "balloon_train", mapper=DatasetMapper(cfg, is_train=True), total_batch_size=76)
##dataloader_train = build_detection_train_loader("balloon_train", mapper=custom_mapper, total_batch_size=76)
...
dataset_dicts = get_balloon_dicts(curdir + "/balloon/val")
...
for d in random.sample(dataset_dicts, 1):    
	...
	#print(outts["instances"].pred_classes)
	mapp = custom_mapper(d, filename)
	#dataloader_test = build_detection_test_loader("balloon_val", mapper=mapp)
	##dataloader_test = build_detection_test_loader("balloon_val", mapper=DatasetMapper(dataset_dicts, is_train=False))
	#build_detection_test_loader takes a list of dictionaries as its first argument and DatasetMapper takes a dataset dict in detectron2 format as its first argument
	##outputs = model(dataloader_test)
	#print(outputs["instances"].pred_classes)
	modelclasses = MetadataCatalog.get("balloon_train").thing_classes
	print(modelclasses)
	df = pd.DataFrame(modelclasses, columns=['Model classes'])
	v = Visualizer(im[..., ::-1], MetadataCatalog.get("balloon_train"), scale=0.5, instance_mode=ColorMode.IMAGE_BW)
	out = v.draw_instance_predictions(outputs["instances"].to("cpu"))
	#cv2.imshow('2', out.get_image()[:, :, ::-1])
	#cv2.imshow('2', out.get_image()[:, :, ::-1][:, :, ::-1])
	#cv2.waitKey(10000)
